...
export async function checkPermissionInc(req, permission) {
    if (!req.user || !permission) {
        return false;
    }
    // Дебаг
    console.log(`[PERMISSION CHECK] Checking permission "${permission}" for user ID: ${req.user.id}`);

    // 1. Проверяем массив permissions в объекте пользователя
    if (Array.isArray(req.user.permissions)) {
        const has = req.user.permissions.includes(permission);
        console.log(`[PERMISSION CHECK] Permission "${permission}" in user.permissions: ${has}`);
        return has;
    }
...
export async function hashPassword(password) {
  const type = process.env.PASSCRYPT_TYPE || 'argon2'; // дефолт argon2
  if (type === 'bcrypt') {
